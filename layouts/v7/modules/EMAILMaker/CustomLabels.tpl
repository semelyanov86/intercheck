...
<div class="container-fluid" id="CustomLabelsContainer">
    ...
    <br>
    <label class="pull-left themeTextColor font-x-x-large">{vtranslate('LBL_CUSTOM_LABELS',$MODULE)}</label>
    <br clear="all">{vtranslate('LBL_CUSTOM_LABELS_DESC',$MODULE)}
    <hr>
    <br />
    <div class="row-fluid">
        <label class="fieldLabel"><strong>{vtranslate('LBL_DEFINE_CUSTOM_LABELS',$MODULE)}:</strong></label><br>
        <div class="row-fluid">
            <div class="pull-right btn-group">
                <button type="button" class="addCustomLabel btn addButton btn-default" data-url="index.php?module={$MODULE}&view=IndexAjax&mode=editCustomLabel"><i class="icon-plus icon-white"></i>&nbsp;<strong> {vtranslate('LBL_ADD')}</strong></button>
        ...
        <div class="pushDownHalfper">    
            ...
            <thead>
                <tr class="blockHeader">
                    <th style="border-left: 1px solid #DDD !important;" width="30%">{vtranslate('LBL_KEY',$MODULE)}</th>
                    <th style="border-left: 1px solid #DDD !important;" width="50%" colspan="2">{vtranslate('LBL_CURR_LANG_VALUE',$MODULE)} ({$CURR_LANG.label})</th>
                    <th style="border-left: 1px solid #DDD !important;" width="20%" align="center">{vtranslate('LBL_OTHER_LANG_VALUES',$MODULE)}</th>
                </tr>
            </thead>
            <tbody>
            ...
            {foreach key=label_id item=label_value from=$LABELS name=lbl_foreach}
                <tr class="opacity">
                    <td><label class="CustomLabelKey textOverflowEllipsis">{$label_value.key}</label></td>
                    <td style="border-right: 0px;"><label class="CustomLabelValue textOverflowEllipsis">{$label_value.lang_values.$lang_id}</label></td>
                    <td style="border-left: 0px;">
                        <div class="pull-right actions">
                            <a class="editCustomLabel cursorPointer" data-url="index.php?module={$MODULE}&view=IndexAjax&mode=editCustomLabel&labelid={$label_id}&langid={$lang_id}">
                                <i title="{vtranslate('LBL_EDIT_CUSTOM_LABEL','PDFMaker')}" class="fa fa-pencil"></i></a>&nbsp;&nbsp;
            ...
            {foreachelse}
                <tr id="noItemFountTr"><td colspan="3" class="cellText" align="center" style="padding:10px;"><strong>{vtranslate('LBL_NO_ITEM_FOUND',$MODULE)}</strong></td></tr>
            {/foreach}
            </tbody>
        </table>
        </div>
