        ...
        <form name='list' id='listedit' action='' onsubmit="return false;">
            <table id="listview-table" class="table {if $LISTVIEW_ENTRIES_COUNT eq '0'}listview-table-norecords {/if} listview-table">
                ...
                <tr class="listViewContentHeader">
                    ...
                    {if $VERSION_TYPE neq 'deactivate'}<th>{vtranslate("Status")}</th>{/if}
                </tr>
                <tr class="searchRow">
                    ...
                    <th>
                        <input type="text" class="listSearchContributor inputElement" data-field-type="string" name="templatename" data-fieldinfo='{ldelim}"column":"templatename","type":"string","name":"templatename","label":"{vtranslate("LBL_EMAIL_NAME",$MODULE)}"{rdelim}' value="{$SEARCH_FILENAMEVAL}">
                    </th>
                    <th>
                ...
                {foreach item=template name=mailmerge from=$EMAILTEMPLATES}
                    <tr class="listViewEntries" {if $template.status eq 0} style="font-style:italic;" {/if} data-id="{$template.templateid}" data-recordurl="index.php?module={$MODULE}&view=Detail&templateid={$template.templateid}" id="{$MODULE}_listView_row_{$template.templateid}">
                        ...
                        {if $VERSION_TYPE neq 'deactivate'}<td class="listViewEntryValue" {if $template.status eq 0} style="color:#888;" {/if}>{$template.status_lbl}&nbsp;</td>{/if}
                    </tr>
                    {foreachelse}
                    <tr>
...
<br>
<div align="center" class="small" style="color: rgb(153, 153, 153);">{vtranslate("EMAIL_MAKER",$MODULE)} {$VERSION} {vtranslate("COPYRIGHT",$MODULE)}</div>
{/strip}