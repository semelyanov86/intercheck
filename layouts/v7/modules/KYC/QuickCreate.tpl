...
{literal}
    <script>
        var fundAmountModel = jQuery('#VDExternalOffers_editView_fieldName_eo_amount');
        var sellFactor = jQuery('select[name="sell_factor"]');
        var commissionRate = jQuery('select[name="eo_commissionrate"]');
        fundAmountModel.change(doCalc);
        sellFactor.change(doCalc);
        commissionRate.change(doCalc);
        function doCalc() {
            var fundValue = parseFloat(fundAmountModel.val());
            if (!fundValue) {
                fundValue = 0;
            }
            var sellValue = parseFloat(sellFactor.val().replace(",","."));
            if (!sellValue) {
                sellValue = 0;
            }
            var commissionValue = parseFloat(commissionRate.val());
            if (!commissionValue) {
                commissionValue = 0;
            }
            var result = fundValue * sellValue;
            var secResult = commissionValue * fundValue;
            jQuery('#VDExternalOffers_editView_fieldName_rtr_amount').val(result.toFixed(2));
            jQuery('#VDExternalOffers_editView_fieldName_eo_commision').val(secResult.toFixed(2));
        }
    </script>
{/literal}
{/strip}