	...
	<div class="container-fluid">
		...
		<div class="app-list row">
			...
			{if $USER_PRIVILEGES_MODEL->hasModulePermission($CONTACTS_MODULE_MODEL->getId())}
				<div class="menu-item app-item app-item-misc" data-default-url="{$CONTACTS_MODULE_MODEL->getListViewUrl()}">
					<div class="menu-items-wrapper">
						<span class="app-icon-list fa">{$CONTACTS_MODULE_MODEL->getModuleIcon()}</span>
						<span class="app-name textOverflowEllipsis"> {vtranslate('Contacts')}</span>
					</div>
				</div>
			{/if}
			{assign var=KYC_MODULE_MODEL value=Vtiger_Module_Model::getInstance('KYC')}
			{if $USER_PRIVILEGES_MODEL->hasModulePermission($KYC_MODULE_MODEL->getId())}
				<div class="menu-item app-item app-item-misc" data-default-url="{$KYC_MODULE_MODEL->getListViewUrl()}">
					<div class="menu-items-wrapper">
						<span class="app-icon-list fa">{$KYC_MODULE_MODEL->getModuleIcon()}</span>
						<span class="app-name textOverflowEllipsis"> {vtranslate('KYC', 'KYC')}</span>
					</div>
				</div>
			{/if}
			{assign var=TRANSACTIONS_MODULE_MODEL value=Vtiger_Module_Model::getInstance('Transactions')}
			{if $USER_PRIVILEGES_MODEL->hasModulePermission($TRANSACTIONS_MODULE_MODEL->getId())}
				<div class="menu-item app-item app-item-misc" data-default-url="{$TRANSACTIONS_MODULE_MODEL->getListViewUrl()}">
					<div class="menu-items-wrapper">
						<span class="app-icon-list fa">{$TRANSACTIONS_MODULE_MODEL->getModuleIcon()}</span>
						<span class="app-name textOverflowEllipsis"> {vtranslate('Transactions', 'Transactions')}</span>
					</div>
				</div>
			{/if}
			...
			{if $USER_PRIVILEGES_MODEL->hasModulePermission($POTENTIALS_MODULE_MODEL->getId())}
				<div class="menu-item app-item app-item-misc" data-default-url="{$POTENTIALS_MODULE_MODEL->getListViewUrl()}">
					<div class="menu-items-wrapper">
						<span class="app-icon-list fa">{$POTENTIALS_MODULE_MODEL->getModuleIcon()}</span>
						<span class="app-name textOverflowEllipsis"> {vtranslate('Potentials', 'Potentials')}</span>
					</div>
				</div>
			{/if}
			{assign var=COMMISSIONS_MODULE_MODEL value=Vtiger_Module_Model::getInstance('Commissions')}
			{if $USER_PRIVILEGES_MODEL->hasModulePermission($COMMISSIONS_MODULE_MODEL->getId())}
				<div class="menu-item app-item app-item-misc" data-default-url="{$COMMISSIONS_MODULE_MODEL->getListViewUrl()}">
					<div class="menu-items-wrapper">
						<span class="app-icon-list fa">{$COMMISSIONS_MODULE_MODEL->getModuleIcon()}</span>
						<span class="app-name textOverflowEllipsis"> {vtranslate('Commissions', 'Commissions')}</span>
					</div>
				</div>
			{/if}
			{assign var=ALARMS_MODULE_MODEL value=Vtiger_Module_Model::getInstance('Alarms')}
			{if $USER_PRIVILEGES_MODEL->hasModulePermission($ALARMS_MODULE_MODEL->getId())}
				<div class="menu-item app-item app-item-misc" data-default-url="{$ALARMS_MODULE_MODEL->getListViewUrl()}">
					<div class="menu-items-wrapper">
						<span class="app-icon-list fa">{$ALARMS_MODULE_MODEL->getModuleIcon()}</span>
						<span class="app-name textOverflowEllipsis"> {vtranslate('Alarms', 'Alarms')}</span>
					</div>
				</div>
			{/if}
			<div class="menu-item app-item app-item-misc" data-default-url="index.php?module=Emails&view=List">
				<div class="menu-items-wrapper">
					<span class="app-icon-list fa"><i class="vicon-emailcampaigns" title="Emails"></i></span>
					<span class="app-name textOverflowEllipsis"> {vtranslate('Emails', 'Emails')}</span>
				</div>
			</div>
			...
			{foreach item=APP_NAME from=$APP_LIST}
				{if $APP_NAME eq 'ANALYTICS' or $APP_NAME eq 'MARKETING' or $APP_NAME eq 'SALES'} {continue}{/if}
