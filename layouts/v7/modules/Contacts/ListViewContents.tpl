			...
			<table id="listview-table" class="table {if $LISTVIEW_ENTRIES_COUNT eq '0'}listview-table-norecords {/if} listview-table ">
				...
				<tbody class="overflow-y">
					...
					{foreach item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS}
						...
						{/if}
						{assign var=LISTVIEW_ENTRY_RAWVALUE value=Contacts_ListView_Model::getRestrictedValue($LISTVIEW_HEADER, $LISTVIEW_ENTRY_RAWVALUE, $CURRENT_USER_MODEL)}
						{assign var=LISTVIEW_ENTRY_VALUE value=$LISTVIEW_ENTRY->get($LISTVIEW_HEADERNAME)}
						{assign var=LISTVIEW_ENTRY_TITLE value=$LISTVIEW_ENTRY->getTitle($LISTVIEW_HEADER)}
						{assign var=LISTVIEW_ENTRY_TITLE value=Contacts_ListView_Model::getRestrictedValue($LISTVIEW_HEADER, $LISTVIEW_ENTRY_TITLE, $CURRENT_USER_MODEL)}
						{assign var=LISTVIEW_ENTRY_VALUE value=Contacts_ListView_Model::getRestrictedValue($LISTVIEW_HEADER, $LISTVIEW_ENTRY_VALUE, $CURRENT_USER_MODEL)}
						<td class="listViewEntryValue" data-name="{$LISTVIEW_HEADER->get('name')}" title="{$LISTVIEW_ENTRY_TITLE}" data-rawvalue="{$LISTVIEW_ENTRY_RAWVALUE}" data-field-type="{$LISTVIEW_HEADER->getFieldDataType()}">
