...
class ModuleLinkCreator_Module_Model extends ModuleLinkCreatorModule_Module_Model
{
    ...
    {
        $vTELicense = new ModuleLinkCreator_VTELicense_Model("ModuleLinkCreator");
        return $vTELicense->validate();
    }
    /**
    ...
    public function getRelation_1_M()
    {
        ...
        if ($adb->num_rows($result)) {
            while ($row = $adb->fetch_array($result)) {
                $relations[] = array("relation_id" => $row["relation_id"], "tabid" => $row["tabid"], "name" => $row["name"], "sequence" => $row["sequence"], "label" => $row["label"], "tab_name" => $row["tab_name"], "tab_tablabel" => vtranslate($row["tab_tablabel"], $row["tab_name"]), "related_tabid" => $row["related_tabid"], "related_tab_name" => $row["related_tab_name"], "related_tab_tablabel" => vtranslate($row["related_tab_tablabel"], $row["related_tab_name"]));
            }
        }
        foreach ($relations as $rel) {
