...
class ITS4YouReports_KeyMetrics_Action extends Vtiger_Action_Controller {
	 ...
	 */
	function addwidget(Vtiger_Request $request) {
		$moduleName = $request->getModule();
    ...
    function addkeymetricsrow(Vtiger_Request $request){
        $moduleName = $request->getModule();
        ...
        if($updatemode!="" && $id!=""){
            ...
            if($updatemode=="create"){
                $sql = "INSERT INTO its4you_reports4you_key_metrics_rows (km_id, label, reportid, calculation_type, column_str, sequence) values (?, ?, ?, ?, ?, ?)";
        ...
        $info = $sql." $label $calculation_type $column_str";
        //$result = array('success' => true, 'message' => vtranslate('LBL_KeyMetricsRow_SAVED', $moduleName), 'info' => $KeyMetricsModel->getInfoArray());
