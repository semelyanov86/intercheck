...
class EMAILMaker_EMAILMaker_Model extends Vtiger_Module_Model
{
    ...
    public function getRecordsEmails($sourceModule, $recordIds, $basic = "")
    {
        ...
        if (count($Emails) > 0) {
            $emailFields[] = array("crmid" => $crmid, "module" => $sourceModule, "data" => $source_data, "emails" => $Emails);
        }
            ...
            if ($numrows > 0) {
                for ($i = 0; $i < $numrows; $i++) {
                    ...
                    if ($single_record) {
                        ...
                        if ($related_id == "" || $related_id == "0") {
                            continue;
                        }
                        if (Vtiger_Util_Helper::checkRecordExistance($related_id) == "1") {
                            continue;
                        }
        ...
        while ($row = $this->db->fetchByAssoc($UsersRes)) {
            ...
            if (count($AUser_Emails) > 0) {
                $Emails_Types["logged"][] = array("crmid" => $user_id, "module" => "Users", "data" => $U_Source_Data, "emails" => $AUser_Emails, "label" => vtranslate($row["fieldlabel"], $sourceModule));
            }
        }
