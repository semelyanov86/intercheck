...
class EMAILMaker_Detail_View extends Vtiger_Index_View {
    ...
    public function preProcess(Vtiger_Request $request, $display = true) {
        ...
        if ($request->has('record') && !$request->isEmpty('record')) {
            ...
            if (substr($recordId, 0, 1) == "t") $recordId = substr($recordId, 1);
    ...
    public function showModuleDetailView(Vtiger_Request $request) {
        ...
        if ($request->has('record') && !$request->isEmpty('record')) {
            ...
            if (substr($record, 0, 1) == "t") $record = substr($record, 1);
            if (!$this->record) {
    ...
    function showDocuments(Vtiger_Request $request) {
        $viewer = $this->getViewer($request);
        $record = $request->get('record');
        if (substr($record, 0, 1) == "t") $record = substr($record, 1);
    ...
    function showEmailCampaigns(Vtiger_Request $request) {
        $viewer = $this->getViewer($request);
        $EMAILMaker = new EMAILMaker_EMAILMaker_Model();
    ...
    function showEmailWorkflows(Vtiger_Request $request) {
        $viewer = $this->getViewer($request);
        $EMAILMaker = new EMAILMaker_EMAILMaker_Model();
    ...
    function getHeaderScripts(Vtiger_Request $request) {
        ...
        $moduleName = $request->getModule();
        $jsFileNames = array("layouts.v7.modules.Vtiger.resources.List", "layouts.v7.modules.Vtiger.resources.Detail", "layouts.v7.modules.EMAILMaker.resources.Detail", "layouts.v7.modules.Vtiger.resources.RelatedList");
        if (vtlib_isModuleActive("ITS4YouStyles")) {
