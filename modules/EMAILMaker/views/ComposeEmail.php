...
class EMAILMaker_ComposeEmail_View extends Vtiger_ComposeEmail_View {
    ...
    public function composeEMAILMakerData($request){
        ...
        if (!$more_sources) $viewer->assign('SOURCERECORD', $RecordId);
        ...
        if (count($SourceIds) > 0){
            foreach ($SourceIds AS $sid){
                if ($nogruping) $groupid = "0";
                else $groupid = $sid;

                foreach($allfieldLists as $listtype => $fieldLists){
                    if (!isset($toMailInfo[$listtype][$groupid])) $toMailInfo[$listtype][$groupid] = array();
                    if (!isset($allMailNamesList[$listtype][$groupid])) $allMailNamesList[$listtype][$groupid] = array();
        ...
        foreach(array("to","cc","bcc") AS $t){
            if(count($to[$t]) > 0) $viewer->assign(strtoupper($t), $to[$t]);
