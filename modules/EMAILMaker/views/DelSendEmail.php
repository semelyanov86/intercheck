    ...
    public function composeMailData(Vtiger_Request $request){		
        ...
        if ($ispdfactive == "1"){    
            ...
            if (count($PDFTemplateIds) > 0) {
                ...
                if ($pdflanguage == "") $pdflanguage = $language;
        ...
        if (count($SourceIds) > 0){    
            foreach ($SourceIds AS $sid){
                ...
                if ($nogruping) $groupid = "0"; else $groupid = $sid;
                if (!isset($toMailInfo[$groupid])) $toMailInfo[$groupid] = array();
                if (!isset($toMailCCInfo[$groupid])) $toMailCCInfo[$groupid] = array();
                if (!isset($toMailBCCInfo[$groupid])) $toMailBCCInfo[$groupid] = array();
                if (!isset($toMailNamesList[$groupid])) $toMailNamesList[$groupid] = array();
                if (!isset($toMailCCNamesList[$groupid])) $toMailCCNamesList[$groupid] = array();
                if (!isset($toMailBCCNamesList[$groupid])) $toMailBCCNamesList[$groupid] = array();
                
                if ($selected_sourceid == "") $selected_sourceid = $sid;
                $recordSorceModel = Vtiger_Record_Model::getInstanceById($sid);
                if (!$nogruping) $SourceNames[$sid] = $recordSorceModel->getName();
				...
				$recordModel = Users_Record_Model::getInstanceFromUserObject($ufocus);
                                ...
                                if (Vtiger_Util_Helper::checkRecordExistance($R_RecordId) == "1") continue;
        ...
        if ($is_listview){    
            ...
            if ($templateid != "" && $templateid != "0"){
                ...
                if ($TemplateModel){
                    ...
                    if ($RecordId != "") $EMAILContentModel->getContent(false);
