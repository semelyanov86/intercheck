...
class EMAILMaker_SendEmails_View extends Vtiger_Index_View {
    ...
    function sendEMAILMakerEmails($from_email_field, $current_user, $ED, $language){
        ...
        if ($num_rows2 > 0){
            while($row2 = $adb->fetchByAssoc($result2)){
                ...
                if ($is_updated == "1"){
                    ...
                    if ($mycrmid == "email"){
                        if ($email_address == "") $email_address = $temp;
                        ...
                        $saved_toid = "";
                        if($temp == "-1"){
                            if ($email_address == "") $email_address = $adb->query_result($adb->pquery("select email1 from vtiger_users where id=?", array($mycrmid)),0,'email1');
                            $user_full_name = getUserFullName($mycrmid);
                            $saved_toid = $user_full_name."<".$email_address.">";
                        } else {
                            if ($mycrmid != ""){
                                if ($email_address == "") $email_address = $this->EMAILMaker->getEmailToAdressat($mycrmid,$temp,$rmodule);
                    ...
                    unset($focus);
                    if ($email_sending_pause > 0) sleep($email_sending_pause);
    ...
    function send_mail($emailadd,$from_name,$from_email,$from_email_field,$subject,$body,$cc,$bcc,$parent_id,$logo,$Email_Images,$phpmailer_version){
    ...
    function getMERecipientEmail($pmodule,$row7){
        $to_email = "";
    ...
    function controlIfIsSent($emailid){
        $adb = PearDatabase::getInstance();
        $adb->setDebug(true);
    ...
    function SaveAttachmentIntoEmail($current_user,$id,$file_name,$filetype,$filetmp_name){
        $adb = PearDatabase::getInstance();
        $adb->setDebug(true);
