...
class EMAILMaker_MassSaveAjax_View extends Vtiger_Footer_View {
    ...
    public function massSave(Vtiger_Request $request){
        ...
        if(is_array($existingAttachments)){
            foreach ($existingAttachments as $index =>  $existingAttachInfo){
                ...
                if(EMAILMaker_EMAILMaker_Model::isStoredName()) {
	                ...
	                $result = $adb->pquery("insert into vtiger_attachments(attachmentsid, name, storedname, description, type, path) values(?, ?, ?, ?, ?, ?)", $params2);
                } else {
        ...
        if($request->get('signature') == 'Yes'){
            ...
            if($signature != '') {
                $S_Data['description'] .= '<br><br>'.decode_html($signature);
            }
        }
        ...
        if ($request->has('selected_sourceid')) {
            $selected_sourceid = $request->get('selected_sourceid');
            if ($selected_sourceid == "0") {
                $getEmails = array('0');
            }
        }
        foreach($getEmails AS $sid) {
            foreach($ETypes AS $etype) {
                $n = $sid.$etype.'emailinfo';

                if ($request->has($n) && !$request->isEmpty($n)) {
                    $toMailNamesList = $request->get($n);
                    foreach ($toMailNamesList AS $pidemail => $e){
                        //list($pid,$pemail) = explode("|",$pidemail,2);
                        $SendMail[$etype][$sid][$pidemail] = $e;
                    }
                }
            }
        }
        $esentid = $this->saveEmails($Send_Emails,$SendMail,$language,$pdf_template_ids,$pdflanguage,$ids_for_pdf);
        if ($esentid){
            $EMAILMaker = new EMAILMaker_EMAILMaker_Model();
            $EMAILMaker->sendEmails($esentid, $request);
    ...
    public function saveEmails($Send_Emails,$SendMail,$language,$pdf_template_ids,$pdflanguage,$ids_for_pdf){
        ...
        if (count($Send_Emails) > 0){
            foreach ($Send_Emails AS $SED){
                ...
                $result = $adb->pquery($sql,array($this->from_name,$this->from_email,$SED["subject"],$SED["description"],'1',$ids_for_pdf,$pdf_template_ids,$pdflanguage,$current_user_id,$SED["att_documents"],"","","","0",$related_to,$pmodule,$language));
                $esentid = $adb->database->Insert_ID("vtiger_emakertemplates_sent");
                $total_emails = 0;
                $CEmails = array();
                $Email_IDs_Array = array();
                        
                foreach($SendMail["to"] AS $sid => $PidsEmails) {
                            ...
                            foreach($SendMail[$c_type][$sid] AS $ccid => $ccemail) {
                                if (is_array($ccemail)) {
                                    foreach ($ccemail AS $ce) {
                                        if (!in_array($ce, $CEmails[$sid][$c_type])) $CEmails[$sid][$c_type][] = $ce;
                                    }
                                } else {
                            ...
                            foreach ($Email_IDs_Array[$sid]["cc"] AS $email_crm_id) {
                                if (!in_array($email_crm_id,$Inserted_Emails)) {
                                    ...
                                    $rel_sql_2 = 'insert into vtiger_seactivityrel values(?,?)';
                                        ...
                                        if (is_numeric($email_crm_id)) $adb->pquery($rel_sql_2,$rel_params_2);
                                }
                            }
                        }
                        if (count($Email_IDs_Array[$sid]["bcc"]) > 0) {
                            foreach ($Email_IDs_Array[$sid]["bcc"] AS $email_crm_id) {
                                if (!in_array($email_crm_id,$Inserted_Emails)) {
                                    ...
                                    if (is_numeric($email_crm_id)) $adb->pquery($rel_sql_3,$rel_params_3);
                                }    
                            }
                        }
    ...
    public function getRecordsListFromRequest(Vtiger_Request $request){
        ...
        if(!empty($selectedIds) && $selectedIds != 'all'){
                if(!empty($selectedIds) && count($selectedIds) > 0) {
                    if (!is_array($selectedIds)) $selectedIds = trim($selectedIds,'"');
                    return $selectedIds;
                }
        }
