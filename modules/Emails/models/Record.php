...
//		return 'Javascript:Vtiger_Index_Js.showEmailPreview("'.$this->getId().'","'.$parentId.'")';
    ...
    public static function getRestrictedValue($header, $value, $user)
    {
        if ($header->getName() == 'from_email' || $header->getName() == 'saved_toid') {
            return self::getPermissionValue($value, $user);
        } else {
            return $value;
        }
    }
