...
class Transactions extends Vtiger_CRMEntity {
	 ...
	 */
	var $customFieldTable = Array('vtiger_transactionscf', 'transactionid');
	...
	var $tab_name_index = Array(
		...
		'vtiger_transactions'   => 'transactionid',
	    'vtiger_transactionscf' => 'transactionid');

	/**
	 * Mandatory for Listing (Related listview)
	 */
	var $list_fields = Array (
        'Transaction Date' => Array('vtiger_transactions'=>'transaction_date'),
                'Transaction No'=>Array('vtiger_transactions'=>'transaction_no'),
                'Transaction type'=>Array('vtiger_transactions'=>'transaction_type'),
                'Payer'=>Array('vtiger_transactions'=>'payer'),
                'Amount'=>Array('vtiger_transactions'=>'amount'),
	...
	var $list_link_field = 'transaction_date';
	...
	var $popup_fields = Array('transaction_date');

	// Placeholder for sort fields - All the fields will be initialized for Sorting through initSortFields
	var $sortby_fields = Array();

	// For Alphabetical search
	var $def_basicsearch_col = 'transaction_date';

	// Column value to use on detail view record text display
	var $def_detailview_recname = 'transaction_date';

	// Required Information for enabling Import feature
	var $required_fields = Array('transaction_date'=>1);

	var $default_order_by = 'transaction_no';
	var $default_sort_order='ASC';
	// Used when enabling/disabling the mandatory fields for the module.
	// Refers to vtiger_field.fieldname values.
	var $mandatory_fields = Array('createdtime', 'modifiedtime', 'transaction_no');
	
    var $related_tables = Array('vtiger_transactionscf' => Array('transactionid')); 
    
	function __construct() {
		global $log;
                $this->column_fields = getColumnFields('Transactions');
	...
	function vtlib_handler($modulename, $event_type) {
		...
		if($event_type == 'module.postinstall') {		
            ...
            $modFocus = CRMEntity::getInstance('Transactions');
            $modFocus->setModuleSeqNumber('configure', 'Transactions', '', '1');
            		// Enable Comments
		...
		$commentRelatedToFieldInstance->setRelatedModules(array($module->name));
            ...
            $emm->removeEntityMethod('Transactions', 'Create Commission from Transaction');
            $emm->removeEntityMethod('Transactions', 'Copy Client Status');
            $emm->removeEntityMethod('Transactions', 'Calculate Balance');
            $emm->removeEntityMethod('Transactions', 'Change Contact Type');
            self::deactiveScheduler();
		} else if($event_type == 'module.enabled') {
                        $this->process_Payments($modulename);
            require 'modules/com_vtiger_workflow/VTEntityMethodManager.inc';
            $emm = new VTEntityMethodManager($adb);
            $emm->addEntityMethod("Transactions", "Create Commission from Transaction","modules/Transactions/workflow/createCommission.php", "createCommission");
            $emm->addEntityMethod("Transactions", "Copy Client Status","modules/Transactions/workflow/copyClientStatus.php", "copyClientStatus");
            $emm->addEntityMethod("Transactions", "Calculate Balance","modules/Transactions/workflow/calcBalance.php", "calcBalance");
            $emm->addEntityMethod("Transactions", "Change Contact Type","modules/Transactions/workflow/changeContactType.php", "changeContactType");
            self::createScheduler();
                        ...
                        for ($i=0; $i<count($rel_modules); $i++) {
                            ...
                            if ($query_tab_id) {
                                if ($adb->num_rows($query_tab_id) > 0) {
                                    ...
                                    $params_links = array($link_id, $tab_id, 'DETAILVIEWBASIC', 'LBL_ADD_TRANSACTION',
                                        'index.php?module=Transactions&action=EditView&return_module=$MODULE$&return_action=DetailView&return_id=$RECORD$&parent_id=$RECORD$',
    ...
    private function createScheduler()
    {
        ...
        $sql = "SELECT id FROM `vtiger_cron_task` WHERE `module` = 'Transactions'";
        $res = $adb->pquery($sql, array());
        if (!$adb->num_rows($res)) {
            $adb->pquery("INSERT INTO `vtiger_cron_task` (`name`, `handler_file`, `frequency`, `status`, `module`, `sequence`) VALUES ('Balance calculation', 'modules/Transactions/cron/Transactions.service', '1800', '0', 'Transactions', '50')", array());
        }
    }
    private function deactiveScheduler()
    {
        $adb = PearDatabase::getInstance();
        $adb->pquery("UPDATE `vtiger_cron_task` SET `status`='1' WHERE (`module`='Transactions')", array());
    }
    private function deleteScheduler()
    {
        $adb = PearDatabase::getInstance();
        $adb->pquery("DELETE FROM `vtiger_cron_task` WHERE (`module`='Transactions')", array());
    }
}
?>