...
class Settings_CloudPBX_Record_Model extends Settings_Vtiger_Record_Model {

    const settingsTable = 'vtiger_cloud_pbxintegration_settings';
    const defaultProvideTable = 'vtiger_cloud_pbx_default_provider';
    ...
    public static function getInstance() {
        ...
        $instance = new Settings_CloudPBX_Record_Model();
    ...
    public static function getClick2SellApiURL() {
        return self::getVoipSettingsFieldValue('click2sell_url');
    }

    public static function getClick2SellLogin() {
        return self::getVoipSettingsFieldValue('click2sell_user');
    }

    public static function getRostelecomSipURI() {
        return self::getRostelecomVoipSettingsFieldValue('cloudrostelecom_extension_sipiru');
    }
    ...
    public static function getYandexUserOutgoing() {
        ...
        $outgoingNumber = null;
        if ($currentUser) {
            $outgoingNumber = $currentUser->get('cloudyandex_outgoing_number');
        }
    ...
    public static function checkRostelecomSignRequest($clientId, $headerSign) {
        ...
        $signKey = Settings_CloudPBX_Record_Model::getRostelecomSignKey();
        $dataNeedToSign = $clientId . $postData . $signKey;
        $sign = Settings_CloudPBX_Record_Model::signRostelecomData($dataNeedToSign);
        if ($sign == $headerSign) {
            return true;
        }
        return false;
    ...
    public static function signRostelecomData($data, $signResponse = false) {
        ...
        if ($signResponse) {
            $clientId = Settings_CloudPBX_Record_Model::getRostelecomIdentificationKey();
            $signKey = Settings_CloudPBX_Record_Model::getRostelecomSignKey();
    ...
    public static function getRostelecomVoipSettingsFieldValue($voipSetting) {
        ...
        $allowedFields = array(
            'cloudrostelecom_extension_internal',
            'cloudrostelecom_extension',
            'cloudrostelecom_extension_sipiru'
        );
