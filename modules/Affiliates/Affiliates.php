...
class Affiliates extends Vtiger_CRMEntity {
    var $db;
	var $table_name = 'vtiger_affiliates';
	var $table_index= 'affiliatesid';
	var $related_tables = Array ('vtiger_affiliatescf' => Array ( 'affiliatesid', 'vtiger_affiliates', 'affiliatesid' ),);


	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_affiliatescf', 'affiliatesid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_affiliates', 'vtiger_affiliatescf');
	...
	var $tab_name_index = Array(
		...
		'vtiger_affiliates' => 'affiliatesid',
		'vtiger_affiliatescf'=>'affiliatesid');
	...
	*/
	function vtlib_handler($moduleName, $eventType) {
		global $adb;
 		if($eventType == 'module.postinstall') {			
			...
			$this->createRelationship('Contacts');
			$this->addRelatedList('Contacts', $moduleName, 'Contacts', array('add'));
    ...
    function get_emails($id, $cur_tab_id, $rel_tab_id, $actions=false) {
        ...
        $query = "SELECT CASE WHEN (vtiger_users.user_name NOT LIKE '') THEN $userNameSql ELSE vtiger_groups.groupname END AS user_name,
                ...
                vtiger_seactivityrel.crmid as parent_id FROM vtiger_activity, vtiger_seactivityrel, vtiger_affiliates, vtiger_users,
                ...
                vtiger_affiliates.affiliatesid = vtiger_seactivityrel.crmid AND vtiger_users.id = vtiger_crmentity.smownerid
                AND vtiger_crmentity.crmid = vtiger_activity.activityid  AND vtiger_affiliates.affiliatesid = $id AND
    ...
    {
        $adb = PearDatabase::getInstance();
        $sql = "SELECT id FROM `vtiger_cron_task` WHERE `module` = 'Affiliates'";
        $res = $adb->pquery($sql, array());
        if (!$adb->num_rows($res)) {
            $adb->pquery("INSERT INTO `vtiger_cron_task` (`name`, `handler_file`, `frequency`, `status`, `module`, `sequence`) VALUES ('Affiliates', 'modules/Affiliates/cron/Affiliates.service', '500', '0', 'Affiliates', '38')", array());
        }
    }
    ...
    {
        $adb = PearDatabase::getInstance();
        $adb->pquery("UPDATE `vtiger_cron_task` SET `status`='1' WHERE (`module`='Affiliates')", array());
    }
    private function deleteScheduler()
    {
        $adb = PearDatabase::getInstance();
        $adb->pquery("DELETE FROM `vtiger_cron_task` WHERE (`module`='Affiliates')", array());
    }
}