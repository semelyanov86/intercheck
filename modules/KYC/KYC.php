...
class KYC extends Vtiger_CRMEntity {
	var $table_name = 'vtiger_kyc';
	var $table_index= 'kycid';
	var $related_tables = Array ('vtiger_kyccf' => Array ( 'kycid', 'vtiger_kyc', 'kycid' ),);


	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_kyccf', 'kycid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_kyc', 'vtiger_kyccf');
	...
	var $list_fields = Array (
		...
		'Document Type' => Array('kyc', 'document_type'),
	...
	var $list_link_field = 'document_type';
	...
	var $popup_fields = Array ('document_type');

	// For Alphabetical search
	var $def_basicsearch_col = 'document_type';

	// Column value to use on detail view record text display
	var $def_detailview_recname = 'document_type';

	// Used when enabling/disabling the mandatory fields for the module.
	// Refers to vtiger_field.fieldname values.
	var $mandatory_fields = Array('document_type','assigned_user_id');

	var $default_order_by = 'document_type';
	...
	*/
	function vtlib_handler($moduleName, $eventType) {
		global $adb;
 		if($eventType == 'module.postinstall') {
			...
			$this->addRelatedList($moduleName, 'Contacts', 'KYC', array('add'));
    ...
    function get_emails($id, $cur_tab_id, $rel_tab_id, $actions=false) {
        ...
        $query = "SELECT CASE WHEN (vtiger_users.user_name NOT LIKE '') THEN $userNameSql ELSE vtiger_groups.groupname END AS user_name,
                ...
                vtiger_seactivityrel.crmid as parent_id FROM vtiger_activity, vtiger_seactivityrel, vtiger_kyc, vtiger_users,
                ...
                vtiger_kyc.kycid = vtiger_seactivityrel.crmid AND vtiger_users.id = vtiger_crmentity.smownerid
                AND vtiger_crmentity.crmid = vtiger_activity.activityid  AND vtiger_kyc.kycid = $id AND
	...
	private function createHandle($moduleName)
	{
        ...
        $emm->addEntityMethod("Contacts", "Create KYC Document","modules/KYC/workflow/createKYC.php", "createKYC");
        $emm->addEntityMethod("KYC", "Change contact status","modules/KYC/workflow/changeContact.php", "changeContact");
	}
	...
	private function removeHandle($moduleName)
	{
        ...
        require 'modules/com_vtiger_workflow/VTEntityMethodManager.inc';
        $emm = new VTEntityMethodManager($adb);
        $emm->removeEntityMethod('Contacts', 'Create KYC Document');
        $emm->removeEntityMethod('KYC', 'Change contact status');
	}
