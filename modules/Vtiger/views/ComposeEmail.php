...
class Vtiger_ComposeEmail_View extends Vtiger_Footer_View {
	...
	public function composeMailData($request){
			...
			if($request->get('emailSource') == 'ListView') {
				...
				foreach($selectIds as $recordId) {
					...
					if($recordModel) {
						...
						foreach($selectedFields as $selectedFieldJson) {
							...
							if(!empty($emailValue)) {
								$to[] = $emailValue;
								$toMailInfo[$recordId][] = $this->clearEmailValue($emailValue);
								$toMailNamesList[$recordId][] = array('label' => decode_html($recordModel->get('label')).' : '.vtranslate('SINGLE_'.$moduleLabel, $moduleLabel), 'value' => $this->clearEmailValue($emailValue));
							}
						}
					}
				}
				...
				foreach ($selectedFields as $selectedFieldJson) {
					...
					if($selectedFieldInfo) {
						...
						$toMailInfo[$selectedFieldInfo['record']][] = $this->clearEmailValue($selectedFieldInfo['field_value']);
						$toMailNamesList[$selectedFieldInfo['record']][] = array('label' => decode_html($selectedFieldInfo['record_label']), 'value' => $this->clearEmailValue($selectedFieldInfo['field_value']));
					}else{
						$fallBack = true;
					}
				}
				  ...
				  if ($id) { 
						...
						if($selectedFields){ 
								foreach($selectedFields as $field) { 
                                        ...
                                        $value = $this->clearEmailValue($value);
        ...
        $newTo = array();
		foreach($to as $value) {
		    $newTo[] = $this->clearEmailValue($value);
        }
		$to = $newTo;
	...
	private function clearEmailValue($value) {
	    $newVal = $value;
        for ($i = -1; $i >= -4; $i--) {
            $lastChar = substr($value, $i);
            if (intval($lastChar) > 0) {
                $newVal = substr($value, 0, $i);
            } elseif ($lastChar === '&') {
                $newVal = substr($value, 0, $i);
            } else {
                return $newVal;
	...
	function emailPreview($request){
		...
		$viewer->assign('FROM', Emails_Record_Model::getPermissionValue($recordModel->get('from_email')));
		$viewer->assign('TO', Emails_Record_Model::getPermissionValue($TO));
