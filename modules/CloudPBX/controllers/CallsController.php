...
class CallsController {
    ...
    private function parseRequest(\Vtiger_Request $request) {
        ...
        if (!empty($clientIdHeader) && !empty($clientSignHeader)) {
            $validRequest = \Settings_CloudPBX_Record_Model::checkRostelecomSignRequest($clientIdHeader, $clientSignHeader);
            if ($validRequest) {
        ...
        if (!empty($token) && $token === \Settings_CloudPBX_Record_Model::getGravitelCrmToken()) {
            return ProvidersEnum::GRAVITEL;
        }

        if (!empty($token) && $token === \Settings_CloudPBX_Record_Model::getMegafonCrmToken()) {
            return ProvidersEnum::MEGAFON;
        }

        if (!empty($token) && $token === \Settings_CloudPBX_Record_Model::getDomruCrmToken()) {
            return ProvidersEnum::DOMRU;
        }

        if (!empty($token) && $token === \Settings_CloudPBX_Record_Model::getWestCallSPBCrmToken()) {
