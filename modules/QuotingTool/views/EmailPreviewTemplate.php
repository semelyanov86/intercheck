...
class QuotingTool_EmailPreviewTemplate_View extends Vtiger_IndexAjax_View
{
    ...
    public function process(Vtiger_Request $request)
    {
        ...
        if ($childModule == "Potentials") {
            $quotingToolRecordModel = new QuotingTool_Record_Model();
            $templates = $quotingToolRecordModel->findByModule("Quotes");
            foreach ($templates as $template) {
                $templateModule = vtranslate($template->get("module"), $template->get("module"));
                $childModule = "";
                if ($template->get("createnewrecords") == 1 && $templateModule != "Quotes") {
                    $childModule = " <i>(" . $templateModule . ")</i> ";
                }
                $relatedChildModuleTemplate[] = array("id" => $template->getId(), "filename" => $fileName = $template->get("filename") . $childModule, "description" => $template->get("description"), "createnewrecords" => $template->get("createnewrecords"), "modulename" => $template->get("module"));
            }
        }
        ...
        $viewer->assign("EMAIL_SUBJECT", strip_tags($record->get("email_subject")));
        ...
        $viewer->assign("CHILD_MODULE", $relModule);
        $viewer->assign("RELATED_CHILD_MODULE_TEMPLATE", $relatedChildModuleTemplate);
        ...
        $viewer->assign("CHECK_ATTACH_FILE", $record->get("check_attach_file"));
