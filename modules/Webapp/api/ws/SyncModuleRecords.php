...
class Webapp_WS_SyncModuleRecords extends Webapp_WS_SaveRecord {

	static $SYNC_MODE_PUBLIC  = "PUBLIC";
	static $SYNC_MODE_PRIVATE = "PRIVATE";

	function isModePrivate(Webapp_API_Request $request, $defmode="PRIVATE") {
		return (strcasecmp($request->get('mode', $defmode), self::$SYNC_MODE_PRIVATE) === 0);
	}

	function process(Webapp_API_Request $request) {
		$current_user = $this->getActiveUser();
		$current_user_wsid = sprintf("%sx%s", Webapp_WS_Utils::getEntityModuleWSId("Users"), $current_user->id);
		...
		if (!empty($lastSyncTime) && $currentPage === 0) {
			...
			if ($hasAssignedToField) {
				if ($isPrivateMode) {
					...
					$andsmowneridequal = Webapp_WS_Utils::querySecurityFromSuffix($module, $current_user);
				}
			}
