...
class Webapp_WS_Utils {
	...
	static function getUserImage($userid){
		global $adb,$site_URL;
	...
	static function getAttachments($fileid,$modcommentsid){
		global $adb,$site_URL;
	...
	static function RecurringDetails($recordid,$module){
	...
	static function getModuleURL($modulename) {
		global $adb,$site_URL;
		$img_url = '';	
		$filename = 'modules/Webapp/MobileIcon/'.$modulename.'.png';
		
		if (file_exists($filename)) {
			$img_url = $site_URL.'modules/Webapp/MobileIcon/'.$modulename.'.png';
		}	
		return $img_url;	
	}
}
