...
class Webapp_WS_FetchRecordsWithGrouping extends Webapp_WS_FetchRecordWithGrouping {
	...
	function process(Webapp_API_Request $request) {
		...
		if (empty($module)) {
			$message = vtranslate('Module not specified.','Webapp');
		...
		// Alert id specified?
			...
			$alert = Webapp_WS_AlertModel::modelWithId($alertid);
			if ($alert === false) {
	...
	function fetchAlertRecords($module, $alert) {		
		...
		if ($module == 'Calendar') {
			$eventsWSID = Webapp_WS_Utils::getEntityModuleWSId('Events');
