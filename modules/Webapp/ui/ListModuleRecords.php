...
class Webapp_UI_ListModuleRecords extends Webapp_WS_ListModuleRecords {
	...
	function getPagingModel(Webapp_API_Request $request) {
		$pagingModel =  Webapp_WS_PagingModel::modelWithPageStart($request->get('page'));
		$pagingModel->setLimit(Webapp::config('Navigation.Limit', 100));
		return $pagingModel;
	}
	...
	function getSearchFilterModel($module, $search) {
		...
		if (!empty($search)) {
			...
			$searchFilter = Webapp_UI_SearchFilterModel::modelWithCriterias($module, $criterias);
			return $searchFilter;
		}
		return $searchFilter;
	}
	...
	function process(Webapp_API_Request $request) {
		...
		if($wsResponse->hasError()) {
			...
			$viewer->assign('_RECORDS', Webapp_UI_ModuleRecordModel::buildModelsFromResponse($wsResponseResult['records']) );
