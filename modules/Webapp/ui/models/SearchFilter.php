...
class Webapp_UI_SearchFilterModel extends Webapp_WS_SearchFilterModel {
	...
	function prepareWhereClause($fieldnames = false) {
		...
		if ($fieldnames === false) {
			foreach($describeInfo['fields'] as $fieldinfo) {
				$fieldmodel = new Webapp_UI_FieldModel();
	...
	static function modelWithCriterias($moduleName, $criterias = false) {
		$model = new Webapp_UI_SearchFilterModel($moduleName);
