...
function createUserSharingPrivilegesfile($userid) {
    ...
    if ($handle) {
        ...
        if ($user_focus->is_admin == 'on') {
            ...
            $newbuf .= "\$Leads_share_read_permission=array('ROLE'=>" . constructTwoDimensionalCharIntSingleValueArray($lead_share_read_per['ROLE']) . ",'GROUP'=>" . constructTwoDimensionalValueArray($lead_share_read_per['GROUP']) . ", 'USER'=>" . constructArray($lead_share_read_per['USER']) .");\n\n";
            $newbuf .= "\$Leads_share_write_permission=array('ROLE'=>" . constructTwoDimensionalCharIntSingleValueArray($lead_share_write_per['ROLE']) . ",'GROUP'=>" . constructTwoDimensionalValueArray($lead_share_write_per['GROUP']) . ", 'USER'=>" . constructArray($lead_share_write_per['USER']) . ");\n\n";
			...
			echo '</pre>';*/
            $newbuf .= "\$Accounts_share_read_permission=array('ROLE'=>" . constructTwoDimensionalCharIntSingleValueArray($account_share_read_per['ROLE']) . ",'GROUP'=>" . constructTwoDimensionalValueArray($account_share_read_per['GROUP'])  . ", 'USER'=>" . constructArray($account_share_read_per['USER']) . ");\n\n";
            $newbuf .= "\$Accounts_share_write_permission=array('ROLE'=>" . constructTwoDimensionalCharIntSingleValueArray($account_share_write_per['ROLE']) . ",'GROUP'=>" . constructTwoDimensionalValueArray($account_share_write_per['GROUP'])  . ", 'USER'=>" . constructArray($account_share_write_per['USER']) . ");\n\n";
            ...
            $newbuf .= "\$Contacts_share_read_permission=array('ROLE'=>" . constructTwoDimensionalCharIntSingleValueArray($contact_share_read_per['ROLE']) . ",'GROUP'=>" . constructTwoDimensionalValueArray($contact_share_read_per['GROUP']) . ", 'USER'=>" . constructArray($contact_share_read_per['USER']) . ");\n\n";
            $newbuf .= "\$Contacts_share_write_permission=array('ROLE'=>" . constructTwoDimensionalCharIntSingleValueArray($contact_share_write_per['ROLE']) . ",'GROUP'=>" . constructTwoDimensionalValueArray($contact_share_write_per['GROUP']) . ", 'USER'=>" . constructArray($contact_share_write_per['USER']) . ");\n\n";
            ...
            $newbuf .= "\$Potentials_share_read_permission=array('ROLE'=>" . constructTwoDimensionalCharIntSingleValueArray($pot_share_read_per['ROLE']) . ",'GROUP'=>" . constructTwoDimensionalArray($pot_share_read_per['GROUP'])  . ", 'USER'=>" . constructArray($pot_share_read_per['USER']) . ");\n\n";
            $newbuf .= "\$Potentials_share_write_permission=array('ROLE'=>" . constructTwoDimensionalCharIntSingleValueArray($pot_share_write_per['ROLE']) . ",'GROUP'=>" . constructTwoDimensionalArray($pot_share_write_per['GROUP'])  . ", 'USER'=>" . constructArray($pot_share_read_per['USER']) . ");\n\n";
            ...
            for ($idx = 0; $idx < count($custom_modules); ++$idx) {
                ...
                $newbuf .= '$' . $module_name . "_share_read_permission=array('ROLE'=>" .
                    ...
                    constructTwoDimensionalArray($mod_share_read_perm['GROUP']) . ", 'USER'=>" . constructArray($mod_share_read_perm['USER']) . ");\n\n";
                $newbuf .= '$' . $module_name . "_share_write_permission=array('ROLE'=>" .
                    ...
                    constructTwoDimensionalArray($mod_share_write_perm['GROUP']) . ", 'USER'=>" . constructArray($mod_share_write_perm['USER']) . ");\n\n";
            }
            // END
...
function getUserModuleSharingObjects($module, $userid, $def_org_share, $current_user_roles, $parent_roles, $current_user_groups) {
    ...
    $mod_share_read_permission['USER'] = Array();
    $mod_share_write_permission['USER'] = Array();
    ...
    if ($def_org_share[$mod_tabid] == 3 || $def_org_share[$mod_tabid] == 0) {
        ...
        /* rs2user start */
        $query = "select vtiger_datashare_rs2user.* from vtiger_datashare_rs2user inner join vtiger_datashare_module_rel on vtiger_datashare_module_rel.shareid=vtiger_datashare_rs2user.shareid where vtiger_datashare_module_rel.tabid=?  and to_userid = ?";
        ...
        /* roles2user start */
        $query = "select vtiger_datashare_role2user.* from vtiger_datashare_role2user inner join vtiger_datashare_module_rel on vtiger_datashare_module_rel.shareid=vtiger_datashare_role2user.shareid where vtiger_datashare_module_rel.tabid=? and to_userid = ?";
        $qparams = array($mod_tabid, $userid);
        $result = $adb->pquery($query, $qparams);
        $num_rows = $adb->num_rows($result);
        for ($i = 0; $i < $num_rows; $i++) {
        ...
        /* grp2user start */
        $query = "select vtiger_datashare_grp2user.* from vtiger_datashare_grp2user inner join vtiger_datashare_module_rel on vtiger_datashare_module_rel.shareid=vtiger_datashare_grp2user.shareid where vtiger_datashare_module_rel.tabid=? and vtiger_datashare_grp2user.to_userid = ?";
        $qparams = array($mod_tabid, $userid);
        $result = $adb->pquery($query, $qparams);
        $num_rows = $adb->num_rows($result);
        for ($i = 0; $i < $num_rows; $i++) {
        ...
        /* user2group start */
        $query = "select vtiger_datashare_user2group.* from vtiger_datashare_user2group inner join vtiger_datashare_module_rel on vtiger_datashare_module_rel.shareid=vtiger_datashare_user2group.shareid where vtiger_datashare_module_rel.tabid=?";
        $qparams = array($mod_tabid);
        if (count($groupList) > 0) {
            $query .= " and vtiger_datashare_user2group.to_groupid in (" . generateQuestionMarks($groupList) . ")";
            array_push($qparams, $groupList);
        }
        $result = $adb->pquery($query, $qparams);
        $num_rows = $adb->num_rows($result);
        for ($i = 0; $i < $num_rows; $i++) {
            ...
            if ($share_permission == 1) {
                if ($def_org_share[$mod_tabid] == 3) {
                    if (!in_array($share_userid, $user_read_per)) {
                        $user_read_per[] = $share_userid;
                    }
                    if (!in_array($share_userid, $user_write_per)) {
                        $user_write_per[] = $share_userid;
                    }
                }
            } elseif ($share_permission == 0 && $def_org_share[$mod_tabid] == 3) {
                if (!in_array($share_userid, $user_read_per)) {
        ...
        /* user2role start */
        $query = "select vtiger_datashare_user2role.* from vtiger_datashare_user2role inner join vtiger_datashare_module_rel on vtiger_datashare_module_rel.shareid=vtiger_datashare_user2role.shareid where vtiger_datashare_module_rel.tabid=? and vtiger_datashare_user2role.to_roleid=?";
        $result = $adb->pquery($query, array($mod_tabid, $current_user_roles));
        $num_rows = $adb->num_rows($result);
        for ($i = 0; $i < $num_rows; $i++) {
            $share_userid = $adb->query_result($result, $i, 'share_userid');
        ...
        /* user2rs start */
        $query = "select vtiger_datashare_user2rs.* from vtiger_datashare_user2rs inner join vtiger_datashare_module_rel on vtiger_datashare_module_rel.shareid=vtiger_datashare_user2rs.shareid where vtiger_datashare_module_rel.tabid=? and vtiger_datashare_user2rs.to_roleandsubid in (" . generateQuestionMarks($parRoleList) . ")";
        $result = $adb->pquery($query, array($mod_tabid, $parRoleList));
        $num_rows = $adb->num_rows($result);
        for ($i = 0; $i < $num_rows; $i++) {
            $share_userid = $adb->query_result($result, $i, 'share_userid');
        ...
        /* user2user start */
        $query = "select vtiger_datashare_user2user.* from vtiger_datashare_user2user inner join vtiger_datashare_module_rel on vtiger_datashare_module_rel.shareid=vtiger_datashare_user2user.shareid where vtiger_datashare_module_rel.tabid=? and vtiger_datashare_user2user.to_userid = ?";
        ...
        $mod_share_read_permission['USER'] = $user_read_per;
        $mod_share_write_permission['USER'] = $user_write_per;
...
function populateSharingPrivileges($enttype, $userid, $module, $pertype, $var_name_arr = false) {
    ...
    if ($enttype == 'USER') {
        ...
        if (sizeof($var_name_arr['USER']) > 0) {
            foreach ($var_name_arr['USER'] as $user_id) {
                if (!in_array($user_id, $user_arr)) {
                    $query = "insert into " . $table_name . " values(?,?,?)";
                    $adb->pquery($query, array($userid, $tabid, $user_id));
                    $user_arr[] = $user_id;
