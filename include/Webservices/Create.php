...
function vtws_create($elementType, $element, $user) {
	...
	if (!in_array($elementType, $types['types'])) {
		throw new WebServiceException(WebServiceErrorCode::$ACCESSDENIED, "Permission to perform the operation is denied for $elementType");
	}
    ...
    foreach ($referenceFields as $fieldName => $details) {
		if (isset($element[$fieldName]) && strlen($element[$fieldName]) > 0) {
			...
			if (!in_array($referenceObject->getEntityName(), $details) && $fieldName != 'roleid') {
